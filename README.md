...
- **4 类意图分流**：knowledge / tool_query / multi_intent / other，LLM 三层防线 + 置信度门控
- **知识库检索组装**：条目 type 分型（info 直接答 / process 流程清单 / index 索引引导），检索层可替换（RAG 演进预留）
- **确定性工具查询**：真 Function Calling → 字段抽取 → 查表 → 模板组装，覆盖教室 / 课表 / 成绩 / 借阅 / 余额 / 失物 / 班车等场景
- **追问澄清 ≤3 轮**：轮次上限图结构硬约束，不靠 LLM 自觉
- **转人工兜底 + 进化闭环**：bad_cases 沉淀 → 管理员审查 → 补入知识库
- **可观测与权限**：Langfuse 全链路埋点（无 key 零开销）；角色默认权限 ∪ 附加权限位
...
| LLM | DeepSeek（deepseek-v4-flash，真 FC 已探测可用）|
...
3. **真 FC 探测先行**：环境验证实测 DeepSeek function calling 可用性（FC_SUPPORTED=True），不写死实现
4. **构建期近重复自动检测**：相似问题在数据构建时自动拦截，保证知识库零冗余
