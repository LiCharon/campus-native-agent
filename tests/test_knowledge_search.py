...
def _seed_scores(session_factory) -> int:
    ...
    M17A 阈值（KEYWORD_MIN_SCORE=4）后，入选须 ≥4 分。搜索"成绩查询"时：
    A–E 均 4 分（各自双关键词命中），F=0 分（无关，被阈值+计分双重排除）。
    同分按插入序稳定排序，A 居首。返回 A 的 id 供断言。
    """
    from campus_desk.db.models import KnowledgeEntry
    ...
    rows = [
        ...
        ("教务", "成绩,绩查", "成绩什么时候出？", "info", "考后两周。"),
        ("教务", "查询,询", "成绩复查流程？", "process", "向教务处申请。"),
        ("教务", "询,成绩", "课表在哪查？", "info", "教务系统→我的课表。"),
        ("教务", "成绩查询,成绩", "成绩查询入口？", "index", "jw 门户。"),
