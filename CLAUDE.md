...
**M11-ZJUT 真实数据采集（✅ 2026-08-25）**：官方公开数据双轨采集（数字 PDF/HTML → pdfplumber/bs4；228 页扫描件手册 → DS 多模态读图 OCR）+ 结构感知条款切块 + build 抽取管道（流程类 LLM 转 FAQ/条文类直存 + 双关卡去重 difflib‖dense 余弦 + 来源标注/核实指引）；知识库 **262→834 条** 11 域全覆盖；**S5 评测 V2**（eval_metrics 全套指标 Recall@1/3/5 + Hit@1 + MRR + nDCG@3 + P@3，评测集 34→70 条多 gold）——834 库 T1 Qdrant混合 R@3=88.3% MRR=0.86 延迟 24.7ms（vs T2 MySQL稠密 207ms，规模化价值兑现）；pytest 397 passed
**以后再说**：真·多意图拆解 / MCP 暴露 / 渠道扩展 / LLM 摘要画像 / SSE 流式
**DoD（完成标准，模式：核心链路测试绿 + 环境验证 + 收尾三件套同步）**：M1-ZJUT 96 passed；M3-ZJUT 166 passed + accept_m3 7/7；M4-ZJUT 180 passed + 运行态 8/8；M5-ZJUT 261 passed + 真实链路冒烟 11/11；M6-ZJUT 273 passed + MySQL 冒烟（三表种子/login JWT 查库/只读接口 401-403-200）；M7-ZJUT 302 passed + MySQL 冒烟（画像落库/role 门控/同进程第二问注入）；M10-ZJUT 313 passed + S5 评测；M9-ZJUT 319 passed + agent-browser 端到端验证（登录→新建→编辑→删除全 200）；M8-ZJUT 330 passed + MySQL 冒烟（4 会话业务指标）+ agent-browser 端到端（看板 6 业务卡 3 图渲染）；M11-ZJUT 397 passed + 834 库入库/向量重建全绿 + S5 V2 全套指标；当前 pytest 397 passed
...
**M11 采集（真实数据）**：浙工大子站对校外 IP 403.6（IIS 白名单，主站开放；VPN 出口也过不了应用级 IP 检查"当前 ip 并非校内地址"）；jwc HTTPS 证书 hostname 不匹配 → requests verify=False + urllib3 抑制；**评测集 gold 是"库快照"断言**——库增条目后旧 gold 不过期更新 → Recall 虚低（834 库下 79.4%→扩集后 88.3%），评测必须随数据演进而重标；多办法合集（手册）有海量模板条款（"第X条…以本办法为准"仅差条款号，difflib 0.97）→ 去重从"终止阻断"改"自动去重保留第一条"+ 模板条款归一化（去条款号）去重；**MySQL question String(256) 上限**（seed 批量入库才 1406 报错）→ build/seed 双层截断（question≤250/keywords≤120）；argparse `action="append"` 只吃单值 → 多值用 `nargs="*"`
